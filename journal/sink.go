...
// Package journal allows sending log messages to systemd journald via logr.Logger.
package journal
...
func NewSink() (*Sink, error) {
	...
	unixConn := conn.(*net.UnixConn) //nolint:forcetypeassert // Panic is wanted.
...
func (s Sink) WithValues(newValues ...interface{}) logr.LogSink { //nolint:ireturn
...
func (s Sink) WithCallDepth(depth int) logr.LogSink { //nolint:ireturn
...
func (s Sink) WithName(name string) logr.LogSink { //nolint:ireturn
	s.values = append(make([]interface{}, 0, len(s.values)), s.values...)
